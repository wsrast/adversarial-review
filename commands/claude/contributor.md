...
argument-hint: <target> [with Codex, Gemini]
---
...
- Assign adversary IDs, create per-agent directories, and write prompts under
  `prompts/`.
- Invoke intended adversaries through available CLIs/integrations.
- Do not invoke Claude as an Adversary when Claude is the Contributor. If Claude
  is named as an adversary, skip it with reason `same-as-contributor`.

...
Most Claude invocations should be worker-mode `/adversary` calls launched by a
Contributor. If invoked as `/contributor`, Claude may orchestrate the same
protocol.
...
session. A Claude Contributor must not call Claude as an Adversary. If the user
names Claude as an adversary while Claude is contributing, record it as skipped
...
4. Write the assigned round file, for example
   `adversaries/01-claude/round-01.md`, or the assigned verification file.
